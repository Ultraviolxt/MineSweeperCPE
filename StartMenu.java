...
public class StartMenu extends MineSweeper {
    ...
    public void showStartMenu() {
        ...
        playButton.addActionListener(_ -> {
            for (int i = 0; i < sizes.length; i++) {
                if (sizeButtons[i].isSelected()) {
                    ...
                    System.out.println("Selected: " + getNumRows() + "x" + getNumCols()); // Debug
                    startFrame.dispose();
                    initGame();
                    return;
                }
        ...
        startFrame.add(bottomPanel, BorderLayout.SOUTH);  // แก้เป็น startFrame.add(bottomPanel, BorderLayout.SOUTH);
